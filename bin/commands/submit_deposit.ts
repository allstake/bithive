...
export const submitDeposit: CommandModule<unknown, Args> = {
  command: "deposit",
  describe: "Submit a BTC deposit transaction",
  builder: {
    env: envBuilder,
    tx: {
      type: "string",
      describe: "Hex encoded BTC deposit transaction",
      demandOption: true,
    },
  },
  async handler({ env, tx }) {
    ...
    // submit mocked data for testing now
    const args = {
      tx_hex: tx,
      deposit_vout: 0,
      embed_vout: 1,
    ...
    console.log("Submitted deposit tx");
  },
};
